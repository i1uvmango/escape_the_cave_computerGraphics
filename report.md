...
*그림 5-2. 손전등(SpotLight)만으로 비춘 통로 — 45° 콘과 거리 감쇠가 드러난다. 글로우스톤(PointLight)·환경광 예시는 Emissive/5장 GI 그림 참고.*
...
*그림 5-3. 손전등 OFF 상태에서 **글로우스톤 블록이 스스로 빛나고(emissive)**, 그 빛이 동굴 표면에 **간접광(GI)** 으로 퍼진 모습. 블록은 의도적으로 무텍스처 발광체다.*
...
- **거리 감쇠(Attenuation)** — 각 광원의 `distance`/`decay`로 거리 제곱에 가까운 감쇠를 적용한다(손전등 사거리 100·감쇠 0.8). 멀어질수록 어두워지는 것이 곧 "빛으로 길을 개척"하는 게임성과 직결된다.

![거리 감쇠](res/attenuation.png)

*그림 5-4. 손전등을 복도에 비스듬히 비춘 컷 — 가까운 벽은 밝고 멀어질수록 어두워지는 **거리 감쇠(attenuation)** 가 보인다.*
...
*그림 5-1. 1인칭에서 보이지 않는 인간 캐릭터(shadow-only)가 **가장 가까운 글로우스톤 방향으로 사람 형태 그림자**를 드리운다. 걸으면 그림자도 함께 걷는다.*
...
>
![DDGI 전파](res/radiance_propagate.gif)
...
| 먼 거리 (낮은 LOD/밉맵 상위) | 가까운 거리 (높은 LOD/원본) |
|---|---|
...
> 📸 **촬영 요청** — 고블린이 걸어올 때 **무릎·팔꿈치·골반이 접히며 메시가 변형되는** 모습을 옆/뒤에서 가까이 찍은 컷(스키닝 변형이 드러나도록).
...
- **리타겟팅/MoCap** — Mixamo 클립은 모션캡처에서 만들어져 **표준 스켈레톤으로 리타겟**된 것으로, 사람·고블린처럼 비율이 다른 캐릭터에 같은 애니메이션을 입힌다.
...
충돌과 GI가 모두 동굴 메시의 BVH를 공유하며, 정적 GI(글로우스톤)는 설치 시 버스트로만 추적해 평상시 비용을 0으로 만든다. (정확한 FPS/드로우콜 수치는 채점 PC 기준으로 측정 후 기입.)
