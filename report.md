...
> 📸 **촬영 요청** — 글로우스톤(PointLight)이 켜진 통로에서 손전등(SpotLight)으로 벽을 비추는 1인칭 컷. 두 광원의 색·범위·감쇠 차이가 한 화면에 보이도록.
...
> 📸 **촬영 요청** — 어두운 곳에서 **글로우스톤 블록 / 초록 출구 / 회전하는 열쇠 조각**이 스스로 빛나는 클로즈업(주변이 어두워 emissive가 도드라지는 컷).
...
> 📸 **촬영 요청** — 손전등을 긴 복도에 비스듬히 비춰 **거리에 따라 밝기가 떨어지는(감쇠)** 그라데이션이 보이는 컷.
...
>
> 📸 **촬영 요청** — **P 키 probe 시각화 상태에서 글로우스톤을 새로 설치**하는 순간을 녹화(간접광이 프로브를 따라 퍼져 수렴하는 과정 = store & reuse가 드러나도록). 약 1~2초 클립.
...
> 📸 **촬영 요청** — **길게 뻗은 복도 바닥/벽을 낮은 각도(grazing angle)** 로 바라본 컷. 먼 쪽 텍스처가 뭉개지지 않고 또렷한지(이방성 필터링 효과)를 보이도록.
