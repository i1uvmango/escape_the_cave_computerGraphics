...
| 적 | 특징 |
|---|---|
| 고블린 | 동굴에 **8마리** 배치. 평소 어둠 속에 정지해 보이지 않음. **손전등 빔에 들어오거나**(시각, 범위 45) **플레이어가 달리는 소리를 들으면**(청각, 범위 30) 플레이어 쪽으로 **달리기 속도의 1/3(2.5)** 로 추격. 접촉 시 1초마다 3 피해. 빛·소리에서 벗어나면 멈춘다. **단, 글로우스톤을 모두 소진하면 빛·소리와 무관하게 모든 고블린이 깨어나 추격·공격한다.** 달리면 빨리 벗어나지만 더 많은 고블린에게 들킨다. |
...
Chase (플레이어 달리기의 1/3 속도로 추격, 복셀 충돌·바닥 스냅)
  ↓ 접촉(거리 < 1.4)
