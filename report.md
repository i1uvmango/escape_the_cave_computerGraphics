...
- **캐스터는 동적 객체만** — 플레이어(경량 휴머노이드)만 `castShadow`(고블린은 비용상 그림자 미적용), 거대한 동굴 메시(약 5만 삼각형)는 `receiveShadow`만 한다. 그림자 패스가 작은 동적 객체만 그리므로 비용이 작다.
...
- **그림자 1광원 제한** — Nearest sampling(`BasicShadowMap`) + 가장 가까운 글로우스톤 1개만 캐스트, 동굴은 receive만(그림자 패스는 플레이어 휴머노이드만 렌더(고블린 제외)).
