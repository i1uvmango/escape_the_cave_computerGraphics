...
- **캐스터는 동적 객체만** — 플레이어(보이지 않는 캡슐 바디)와 고블린만 `castShadow`, 거대한 동굴 메시(약 5만 삼각형)는 `receiveShadow`만 한다. 그림자 패스가 작은 동적 객체만 그리므로 비용이 작다.
- 플레이어 바디는 1인칭에서 보이지 않도록 **카메라를 캡슐 내부에 두고 FrontSide(뒷면 컬링)** 로 처리해, 화면엔 안 보이지만 **글로우스톤 방향으로 바닥/벽에 그림자**를 드리운다.
