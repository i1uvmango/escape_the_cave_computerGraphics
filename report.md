...
2. 각 글로우스톤 위치의 셀에 RGB 광량을 주입한다. **켜져 있는 손전등은 빔이 닿는 지점(레이캐스트)에 매 프레임 광량을 주입**해 움직이는 동적 광원으로 반영한다.
...
글로우스톤은 정적이지만 **손전등은 움직이는 광원**이므로, **매 프레임 격자를 재전파**해 손전등의 동적 간접광까지 실시간 반영한다. 격자가 거칠어(약 4만 셀, 9회 전파) 비용이 충분히 낮으며, 약한 PC에서는 N프레임마다 갱신하도록 조절할 수 있다.
...
- 거친 격자(약 4만 셀, 9회 전파) 덕분에 **매 프레임 재전파해도 비용이 낮아** 움직이는 손전등의 동적 GI까지 실시간 반영하며, 부하 시 N프레임 throttle로 안전하게 낮출 수 있다.
- 복셀 flood 방식이 "모퉁이 너머 간접광"이라는 목표 시각효과를 직관적으로 달성한다.

### 한계점

- 손전등 간접광은 빔 도달 지점에 광량을 주입하는 근사이며, 콘 전체를 적분하는 정밀 방식은 아니다.
