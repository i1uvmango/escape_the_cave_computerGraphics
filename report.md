...
- 플레이어 바디(인간 캐릭터)는 **1인칭에서 shadow-only**(`colorWrite=false, depthWrite=false`)로 처리해 화면엔 안 보이지만 **글로우스톤 방향으로 바닥/벽에 그림자**를 드리운다. **V로 3인칭 전환** 시 같은 바디를 보이게(`colorWrite=true`) 바꿔 캐릭터 뒷모습과 그림자를 함께 보여준다.
...
> GI의 광원은 **글로우스톤(정적 광원)** 이다. 글로우스톤은 한번 놓이면 움직이지 않으므로, **설치 시에만 프로브 격자를 짧게 재계산(버스트)** 하고 평상시에는 프레임당 GI 비용이 0이다. **손전등은 GI 대상이 아니라 일반 직접광원(SpotLight)** 으로만 처리한다(이동 광원을 DDGI에 넣으면 매 프레임 광선추적이 필요해 비용이 과도하기 때문).
...
- **정적 광원 + 버스트 갱신** — 글로우스톤은 움직이지 않으므로 설치 시에만 프레임 분산 버스트로 수렴시키고 평상시 GI 비용은 0이다. 이동 광원(손전등)은 매 프레임 재추적 비용이 커서 GI에 넣지 않고 일반 직접광으로 처리한다.
...
- **플레이어** — 인간 캐릭터 `Standard Walk.fbx`. 1인칭에서는 shadow-only로 그림자만, **V로 3인칭** 전환 시 캐릭터 뒷모습을 보여준다.
...
기본은 1인칭(눈높이 카메라)이고 **V 키로 3인칭** 전환이 가능하다. 3인칭은 플레이어 뒤쪽에 카메라를 두되, 벽에 가려지지 않도록 **카메라→플레이어 사이를 레이캐스트해 거리를 자동 단축**한다(뒷모습이 충분히 보이도록).
