...
> **(개발 노트)** 처음에는 GI를 단순 시각 효과로 넣으려 했지만, 플레이 테스트 결과 "굳이 켤 이유"가 부족했다. 그래서 글로우스톤을 **한정 자원**으로 만들고 GI 자체를 **안전 구역을 만드는 수단**으로 바꿔 게임 규칙에 직접 연결했다.
...
| 적 | 특징 |
|---|---|
| 고블린 | 동굴에 **8마리** 배치. 평소 어둠 속에 정지해 보이지 않음. **손전등 빔에 들어오거나**(시각, 범위 45) **플레이어가 달리는 소리를 들으면**(청각, 범위 30) 플레이어 쪽으로 **달리기 속도의 약 1/3(3.8)** 로 추격. 접촉 시 1초마다 3 피해. 빛·소리에서 벗어나면 멈춘다. **단, 글로우스톤을 모두 소진하면 빛·소리와 무관하게 모든 고블린이 깨어나 추격·공격한다.** 달리면 빨리 벗어나지만 더 많은 고블린에게 들킨다. |
...
## 3. 핵심 기술 — Global Illumination (GI)

> 본 게임의 **핵심 기술은 실시간 간접광(GI)** 이다. 아래는 전체 시스템 구조와, 그 위에서 동작하는 GI 구현이다.
...
> 위 구조 위에서 동작하는 **핵심 기술(GI)** 의 구현은 다음과 같다.
...
>
> **(개발 노트)** 처음에는 정통 DDGI처럼 **매 프레임 프로브를 갱신**하려 했지만, WebGL2 환경에서는 비용이 너무 커 **20fps 이하로 떨어졌다.** 그래서 광원(글로우스톤)이 정적이라는 점을 이용해 **설치할 때만 버스트로 갱신**하는 방식으로 바꿨고, 평상시 GI 비용을 0으로 만들었다.
...
- **캐스터는 동적 객체만** — 플레이어(경량 휴머노이드)와 고블린만 `castShadow`, 거대한 동굴 메시(약 5만 삼각형)는 `receiveShadow`만 한다. 그림자 패스가 작은 동적 객체만 그리므로 비용이 작다.
- 플레이어 바디(경량 휴머노이드)는 **shadow-only**(`colorWrite=false, depthWrite=false`)로 처리한다 — 1인칭 화면엔 보이지 않지만 **글로우스톤 방향으로 바닥/벽에 사람 형태 그림자**를 드리운다.

이와 별도로 **버텍스 AO**(복셀 밀집도 기반)와 **간접광(GI) 가림**(5장)이 동굴 표면의 음영을 보완한다.

> **(개발 노트)** 처음엔 모든 광원이 그림자를 던지게 했더니 점광원 큐브맵(6면) 비용으로 프레임이 무너졌다. **가장 가까운 글로우스톤 1개만** 그림자를 던지도록 줄이고, 거대한 동굴은 receive만 하도록 바꿔 해결했다.

![플레이어 그림자](res/player_shadow.gif)

*그림 5-4. 1인칭에서 보이지 않는 경량 휴머노이드(shadow-only)가 **가장 가까운 글로우스톤 방향으로 사람 형태 그림자**를 드리운다. 걸으면 그림자도 함께 걷는다.*
...
- **적(고블린)** — Mixamo 스켈레탈 캐릭터 `Walking.fbx`를 `FBXLoader`로 불러오고, 여러 마리를 위해 **SkeletonUtils.clone** 으로 스킨드 메시를 정확히 복제한다. 이동 중에만 걷기 클립을 재생하고 멈추면 `AnimationMixer`를 정지한다.
- **플레이어 그림자** — 1인칭이라 본체는 화면에 안 보이지만, **머리·몸통·팔·다리로 구성한 경량 휴머노이드**(코드 생성, 다운로드 0)를 **그림자 caster**로 써서 사람 형태 그림자를 만든다. 이동 시 팔다리를 스윙해 걷는 그림자가 된다. (웹 구동 안정성을 위해 대용량 인간 FBX 의존을 제거)
...
- **시각(손전등 빔)** — 플레이어 시선 콘 안 + 인식 범위(45) 안에 들어오면 활성화.
- **청각(달리는 발소리)** — 플레이어가 **Shift+W로 달리면** 큰 발소리(증폭 사운드)가 나고, **청각 범위(30) 안의 고블린이 깨어나 추격**한다. 걸으면 들키지 않는다.

#### 상태 머신

```text
Idle (정지·비가시)
  ↓ 손전등 빔이 닿거나 달리는 소리를 들음
Chase (플레이어 달리기의 약 1/3 속도로 추격, 복셀 충돌·바닥 스냅)
  ↓ 접촉(거리 < 1.4)
...
> **(개발 노트)** 빠르게 달리면 얇은 벽을 통과하는 **터널링**이 있었다. 한 프레임 이동량이 충돌 반경보다 커서 생기는 문제라, 이동을 **반경 단위로 쪼개 매 스텝 충돌을 처리**하도록 바꿔 해결했다.
...
- 에셋:
  ...
  - 고블린 캐릭터·애니메이션(`Walking.fbx`) — **Mixamo (Adobe)**
